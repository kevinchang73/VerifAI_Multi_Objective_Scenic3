...
for file in all_files:
    ...
    if mode == 'single':
        for i in range(1, len(lines)):
            ...
            ego_brake.append(float(line.split(',')[-7]))
            adv_speed.append(float(line.split(',')[-8]))
            adv1_dist.append(float(line.split(',')[-9]))
    else:
        for i in range(1, len(lines), 3):
            line1 = lines[i]
            ego_speed.append(float(line1.split(',')[-6]))
            ego_brake.append(float(line1.split(',')[-7]))
            adv_speed.append(float(line1.split(',')[-8]))
            adv1_dist.append(float(line1.split(',')[-9]))
